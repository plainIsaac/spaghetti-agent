...
async function refresh(){let view=await (await fetch('/api/view')).json(),work=view.inspection;replies.innerHTML=view.default.replies.map(r=>`<div class=reply><small>${r.sender}</small><br>${r.text}</div>`).join('')||'<p>No replies yet.</p>';document.querySelector('#agents').innerHTML=work.agents.map(a=>`<div class=reply><b>${a.name}</b> · ${a.role} · ${a.phase}${a.pending_messages?' · '+a.pending_messages+' pending':''}</div>`).join('')||'<p>No active agents.</p>';document.querySelector('#tasks').innerHTML=work.tasks.map(t=>`<div class=reply>Task #${t.id} · ${t.owner} · ${t.state}<br>${t.title}</div>`).join('')||'<p>No active tasks.</p>';let branches=work.branches.filter(b=>b.state==='submitted');document.querySelector('#branches').innerHTML=branches.map(b=>`<div class=reply><b>Task #${b.task_id}</b> — ${b.agent}, ${b.files} file(s)<pre>${b.diff||'No changed files'}</pre><button onclick="merge(${b.task_id})">Merge reviewed branch</button></div>`).join('')||'<p>No submitted branches.</p>';inspection.textContent=JSON.stringify({state:view.default.state,...work},null,2)}
